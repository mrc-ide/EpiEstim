...
Note that we do not support the distinction between local and imported cases when using temporally aggregated incidence as we assume that this level of data would not be available.
...
The SI distibution can be specified as as either using the `method` and `config` parameters (see the full_EpiEstim_vignette for more details).
