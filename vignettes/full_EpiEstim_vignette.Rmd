...
Here EL/ER are the lower/upper bounds for the day of symptom onset for the infector and SL/SR are the lower/upper bounds of the day of symptom onset for the infected individual. A symptom onset on a known day `d` is given as a lower bound of `d` and an upper bound of `d + 1`. The last column "type" is optional and corresponds to whether the data is doubly interval-censored (0), single interval-censored (1) or exact observations (2), but if this is not supplied it will automatically be computed from the dates. An optional column "OT" can give the last day on which the symptom onset of the infected individual could have been observed. The estimation then accounts for right truncation, which matters for pairs observed during an ongoing outbreak (Charniga et al. PLoS Comp Biol 2024). 
