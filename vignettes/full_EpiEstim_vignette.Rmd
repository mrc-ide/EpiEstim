...
vignette: >
  ...
  %\VignetteIndexEntry{Full EpiEstim vignette}
...
If a disease has been well characterised, then you have the option to use estimates of the serial interval that can be found in the literature. Note that EpiEstim
assumes the serial interval is strictly positive. You can either supply the mean and standard deviation (parametric distribution) of the serial interval, or its full distribution (non-parametric distribution). See examples A and B in the Examples section. 
...
knitr::kable(df, "html", col.names = c("Day infector symptom onset (LB)",
...
table2 <- knitr::kable(df, "html", col.names = c("Date", "Incidence Overall",
                                                 "Incidence Local", "Incidence Imported"))
...
Here I~t~ represents the incidence of symptom onset at time t, which is approximated by a Poisson process using the renewal equation. R~t~ is what we want to estimate, I~t-s~ is the past incidence and $\omega$ is the serial interval distribution.
...
```{r fig.align = "center", warning = FALSE, echo = FALSE, out.width = "30%"}
x_lower_norm <- 0
x_upper_norm <- 7
ggplot(data.frame(x = c(x_lower_norm , x_upper_norm)), aes(x = x)) + 
  xlim(c(x_lower_norm , x_upper_norm)) + 
  stat_function(fun = dgamma,
                args = list(shape = (3 / 1) ^ 2, rate = 3 / 1 ^ 2)) +
  theme_minimal() +
  theme(
    plot.margin = margin(t = 50, r = 20, b = 20, l = 20),
    axis.title.x = element_text(
      margin = margin(t = 5, r = 0, b = 0, l = 0), size = 13),
    axis.title.y = element_text(
      margin = margin(t = 0, r = 10, b = 0, l = 0), size = 13),
    panel.grid.minor = element_blank(),
    panel.grid.major = element_blank(),
    axis.line = element_line(color = "lightgrey", linewidth = 0.5)) +
...
df <- data.frame(x = seq(0, 6, length.out = 100)) %>%
  mutate(y = dnorm(x, mean = 3, sd = 1))
mean.plot <- ggplot(df, aes(x, y)) +
...
df2 <- data.frame(x = seq(0, 3.5, length.out = 100)) %>%
  mutate(y = dnorm(x, mean = 1.5, sd = 0.5))
stdev.plot <- ggplot(df2, aes(x, y)) +
  geom_area(fill = "sky blue") +
  gghighlight(x > 0.5 & x < 2.5) +
  theme_minimal() +
  theme(
    plot.margin = margin(t = 50, r = 20, b = 20, l = 20),
    axis.title.x = element_text(
...
ggplot(data.frame(x = c(x_lower_norm , x_upper_norm)), aes(x = x)) + 
  ...
  mapply(function(mean, sd) {
    stat_function(fun = dgamma, args = list(
      shape = (mean / sd)^2, rate = mean / sd^2))}, mean = means, sd = sds) +
  theme_minimal() +
  theme(
    plot.margin = margin(t = 50, r = 20, b = 20, l = 20),
    axis.title.x = element_text(
...
<div style="margin-top:30px;">
**From data** - `"si_from_data"` allows you to estimate the SI by maximum likelihood from infector-infected data, using `primarycensored::fitdistdoublecens()`. The user has the choice of which parametric distribution the SI will take, either: Gamma `"gamma"`, Lognormal `"lognormal"`, Weibull `"weibull"`, Exponential `"exponential"`, or one of these shifted by 1, e.g. `"gamma_offset_1"`. The seed, e.g. `seed = 1`, acts as the starting point for the random number generator used to draw the sample of SI distributions. Setting the seed allows you to obtain reproducible results as you will get the same result each time you run the same process.
</div>

This is set in the `config` argument of `estimate_R()`:

```{r, eval=FALSE}
estimate_R(incid = incid, 
           method = "si_from_data",
           si_data = data$si_data, # supply dataframe with symptom onset data
           config = make_config(si_parametric_distr = "gamma", # choose distribution
...
<div style="margin-top:30px;">
**From sample** - (`"si_from_sample"`) allows you to input the posterior sample from a previously estimated SI distribution. This means that you don't have to re-estimate the SI every time you run `estimate_R()` if you don't want to, and you can also estimate the SI elsewhere and put that into EpiEstim. Ex. The user would need to supply `si_sample` with a matrix where each column corresponds to one SI distribution to be explored. Such a sample can be obtained with `primary2estim()` from a serial interval estimated with primarycensored, e.g. with `primarycensored::fitdistdoublecens()` or, for a Bayesian fit, `primarycensored::pcd_cmdstan_model()`. This would look something like this:
</div>

```{r, messages=FALSE, results=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
data("MockRotavirus")
...
linelist <- read.csv("./data_full_vignette/evd_linelist.csv")
```
...
Here EL/ER are the lower/upper bounds for the day of symptom onset for the infector and SL/SR are the lower/upper bounds of the day of symptom onset for the infected individual. A symptom onset on a known day `d` is given as a lower bound of `d` and an upper bound of `d + 1`. The last column "type" is optional and corresponds to whether the data is doubly interval-censored (0), single interval-censored (1) or exact observations (2), but if this is not supplied it will automatically be computed from the dates. An optional column "OT" can give the time up to which symptom onsets of infected individuals are observed, so `d + 1` for pairs observed up to and including day `d`. The estimation then accounts for right truncation, which matters for pairs observed during an ongoing outbreak (Charniga et al. PLoS Comp Biol 2024). 

This can be used to estimate the SI distribution by maximum likelihood:
...
nR <- nrow(R_diff_prior$R)
test$type <- c(rep("Mean prior 2", nR), rep("Mean prior 5", nR))
test$dates <- rep(R_si_parametric$dates[15:84], 2)
...
ggplot(test, aes(dates, `Mean(R)`, group = type, col = type)) +
  ...
  geom_ribbon(aes(ymin = `Quantile.0.025(R)`, ymax = `Quantile.0.975(R)`,
                  fill = test$type), col = NA, alpha = 0.3) +
...
test2$type <- c(rep("1 week window", 70), rep("3 week window", 63))
test2$dates <- c(R_si_parametric$dates[15:84], R_window$dates[22:84])

ggplot(test2, aes(dates, `Mean(R)`, group = type, col = type)) +
  scale_colour_manual(values = c("#f06553", "#3c3b5f"), name = "")+
  geom_ribbon(aes(ymin = `Quantile.0.025(R)`, ymax = `Quantile.0.975(R)`,
                  fill = test2$type), colour = NA, alpha = 0.3) +
  scale_fill_manual(values = c("#ff6961", "#60B7DE"), name = "") +
  geom_line() +
  ylab("R")+
  xlab("Time")
```
