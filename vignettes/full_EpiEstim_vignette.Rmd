...
<div style="margin-top:30px;">
**From data** - `"si_from_data"` allows you to estimate the SI by maximum likelihood from infector-infected data, using `primarycensored::fitdistdoublecens()`. The user has the choice of which parametric distribution the SI will take, either: Gamma `"G"`, Weibull `"W"`, Lognormal `"L"`, or shifted by 1: Gamma `"off1G"`, Weibull `"off1W"`, Lognormal `"off1L"` 

`mcmc_control` is where you set the seed, e.g. `seed = 1`, which acts as the starting point for the random number generator used to draw the sample of SI distributions. Setting the seed allows you to obtain reproducible results as you will get the same result each time you run the same process. The `burnin` and `thin` arguments are no longer used as the SI is no longer estimated by MCMC.
</div>

```{r, eval=FALSE}
mcmc_control <- make_mcmc_control(seed = mcmc_seed) # set the seed for reproducibility
```
...
MCMC_seed <- 1
overall_seed <- 2
SI.fit <- estimate_R(incid = MockRotavirus$incidence,
                     method = "si_from_data",
...
```{r, results='hide', warning=FALSE, message=FALSE, fig.height=7,fig.width=5}
# Set the seed used to draw the sample of SI distributions
mcmc_control <- make_mcmc_control(seed = 1) # set the seed to make the process reproducible
