...
For ii), there are several options, specified in the `method` argument. All 
options assume that the serial interval is strictly positive. 
The simplest option is the `parametric_si` method, where you only specify the 
...
interval (the offset ensuring the serial interval is >0). 
In the following example, we use the mean and standard deviation of the serial 
...
mean and standard deviation, this can be fed into `estimate_r` as demonstrated
below. Note that the serial interval distribution provided must have a 
probability mass of zero for the first time step, ensuring the SI is >0. 
...
additional rounding). The discretisation method implemented in `discr_si` is
documented in the supplementary material of Cori et al. AJE 2013. It assumes
by default a Gamma distributed serial interval shifted by one day, and accounts
for the fact that the serial interval is double-censored, i.e. 
the time of symptom onset in both the primary and secondary cases are
discrete (typically with a daily interval).
`discr_si` uses `primarycensored::dprimarycensored()` for this, which also supports other
distributions, shifts and truncation of the serial interval (see `?discr_si`).
...
estimated, by maximum likelihood using `primarycensored::fitdistdoublecens()`, 
from interval censored exposure data. The reproduction number is then estimated 
using a sample of SI distributions drawn from the uncertainty in this 
estimate, hence accounting for the uncertainty associated with this estimate. 
...
overall_seed <- 2
dist <- "gamma" # fitting a Gamma distribution for the SI
...
## the third plot now shows the sample of SI distributions 
...
and so you don't want to estimate the SI within `estimate_R` every time. 
You can do this by fitting the SI with 
`primarycensored::fitdistdoublecens()` and converting the fit with 
`primary2estim()`, as follows. `primary2estim()` also accepts a Bayesian fit 
from `primarycensored::pcd_cmdstan_model()`.

```{r estimate_r_si_from_sample}
## using the same random seeds as before to be able to compare results

## first estimate the SI distribution by maximum likelihood with
## primarycensored, accounting for double interval censoring, using the same
## starting values as method "si_from_data":
si_data <- Flu2009$si_data
censdata <- data.frame(left = si_data$SL - si_data$EL,
                       right = si_data$SR - si_data$EL,
                       pwindow = si_data$ER - si_data$EL,
                       D = Inf)
SI_fit <- primarycensored::fitdistdoublecens(
  censdata, distr = "gamma", start = si_start_values(si_data, "gamma")
)
## drawing a sample of SI distributions and converting using primary2estim
si_sample <- primary2estim(SI_fit, dist = stats::pgamma, n = config$n1,
                           seed = overall_seed)$si_sample
...
above 
[https://github.com/jstockwin/EpiEstimApp](https://github.com/jstockwin/EpiEstimApp). Please note the app does not have all features from the R package. 
