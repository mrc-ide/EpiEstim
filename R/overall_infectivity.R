...
#' incidence vector \eqn{I}, with `I = incid$local + incid$imported` if
#' \eqn{I} is a matrix), weighted by their infectivity at time \eqn{t} (given by
#' the discrete serial interval distribution \eqn{w_k}). 
#' 
...
#'   `incid$local` contains the incidence of cases due to local transmission and
#'   `incid$imported` contains the incidence of imported cases (with
#'   `incid$local + incid$imported` the total incidence).
#'
