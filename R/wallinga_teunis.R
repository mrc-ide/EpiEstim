...
#' \itemize{
#'
#' \item Vector (or a dataframe with a column named 'incid') of non-negative
#'   integers containing an incidence time series. If the dataframe contains a
#'   column \code{incid$dates}, this is used for plotting. \code{incid$dates}
...
#'   \code{t_start[i]<=t_end[i]}. t_start[1] should be strictly after the first
#'   day with non null incidence.
#'
...
#'
#' \item mean_si: For method "parametric_si"; positive real giving the mean
#'   serial interval.
#'
#' \item std_si: For method "parametric_si" ; non negative real giving the
#'   stadard deviation of the serial interval.
#'
...
#'   should be zero.
#'
#' \item n_sim: A positive integer giving the number of simulated epidemic trees
#'   used for computation of the confidence intervals of the case reproduction
#'   number (see details).
#'
#' \item plot: Logical. If \code{TRUE} (default is \code{FALSE}), output is
#'   plotted (see value).
#'
#' \item legend: A boolean (TRUE by default) governing the presence /
...
#' \itemize{
#'
#' \item R a dataframe containing the times of start and end of each time window
#'   considered ; the estimated mean, std, and 0.025 and 0.975 quantiles of the
#'   reproduction number for each time window.
#'
#' \item si_distr a vector
#'   containing the discrete serial interval distribution used for estimation
#'
...
#'   predefined time windows can be obtained, for a given discrete distribution
#'   of the serial interval, as proposed by Wallinga and Teunis (AJE, 2004).
#'   Confidence intervals are obtained by simulating a number
#'   (\code{config$n_sim}) of possible transmission trees.
#'
