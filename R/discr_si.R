...
#' assuming that the serial interval is a shifted Gamma distributed, 
#' with shift 1.
#' 
...
{
  if (sigma < 0) {
    cli::cli_abort("{.var sigma} must be >=0.")
  }
  if (mu <= 1) {
    cli::cli_abort("{.var mu} must be >1")
  }
  if (any(k < 0)) {
    cli::cli_abort("all values in {.var} must be >=0.")
  }
