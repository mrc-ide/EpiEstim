...
#'   parameters to use for the SI distribution. This is the shape and scale 
#'   for all but the lognormal distribution, for which it is the meanlog and 
...
#'   \item{thin}{a positive integer corresponding to thinning parameter; the 
#'   MCMC will be run for \code{burnin+n1*thin iterations}; 1 in \code{thin} 
...
#' The second one shows the posterior mean and 95\% credible interval of the 
#' reproduction number. The estimate for a time window is plotted at the end of 
#' the time window.
...
#'                              plot = TRUE))
#' ## generate plots
#' p_uncertainty <- plots(R_si_from_data, "R", 
#'      options_R = list(ylim = c(0, 1.5)))
...
#' @importFrom stats median pgamma plnorm pweibull qgamma qlnorm quantile 
#' qweibull rgamma rmultinom rnorm sd
...
estimate_R_func <- function(incid,
  ...
  if (si_uncertainty == "Y") {
    if (parametric_si == "Y") {
      ...
      std_si_sample <- rep(-1, config$n1)
      for (k in seq_len(config$n1)) {
        mean_si_sample[k] <- sum((seq_len(dim(si_sample)[1]) - 1) * si_sample[, k])
