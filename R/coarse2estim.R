#######################################################################################################################
# coarse2estim integates CoarseDataTools with EpiEstim using the amended version of EstimateR called estimate_r #
#######################################################################################################################

#' Link coarseDataTools and EpiEstim
#' 
#' \code{coarse2estim} Transforms outputs of \code{coarseDataTools::dic.fit.mcmc} to right format for input into \code{estimate_r}
#' 
...
#' #' Should be one of "G" (Gamma), "W" (Weibull), "L" (Lognormal), "off1G" (Gamma shifted by 1), "off1W" (Weibull shifted by 1), or "off1L" (Lognormal shifted by 1).  If not present, computed automatically from \code{x}.
...
coarse2estim <- function(x=NULL, dist=x@dist, samples=x@samples, thin=10){
  ...
  if(is.null(x)) # then check that dist and samples are what we expect
  {
    ...
    {
      stop("The supported distributions are 'G' (Gamma), 'W' (Weibull), 'L' (Lognormal), 'off1G' (Gamma shifted by 1), 'off1W' (Weibull shifted by 1), or 'off1L' (Lognormal shifted by 1). ")
    }
